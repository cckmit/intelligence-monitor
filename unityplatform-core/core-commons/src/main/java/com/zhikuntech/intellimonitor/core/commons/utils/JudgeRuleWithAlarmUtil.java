...
public class JudgeRuleWithAlarmUtil extends JudgeRuleWithAlarmUtils {
    ...
    public static AlarmResultDTO process(BigDecimal num, AlarmRuleDTO alarmRule) {
        ...
        //如果交集连接
        if(alarmRange.indexOf(ANDSET) > -1){
            alarmRangeSetSplit = alarmRange.split(String.valueOf(ANDSET));
        }
        //如果并集连接
        else if(alarmRange.indexOf(INTERSET) > -1){
    ...
    private static boolean judgeSingle(Set<MeasuringPoint[]> set, BigDecimal pvalue){
        for(MeasuringPoint[] measuringPoint:set){
            boolean contain_left = measuringPoint[0].contain;
            if(contain_left){
                //判断左边的值(包含)
                //大于等于
                if(pvalue.compareTo(measuringPoint[0].value) > -1){
                    boolean contain_right = measuringPoint[1].contain;
                    //判断右边的值
                    boolean jugdeRightResult = jugdeRight(measuringPoint[1].value, pvalue, contain_right);
                    if(jugdeRightResult){
                        return true;
                    }
                }
    ...
    //判断右边的值
    private static boolean jugdeRight(BigDecimal rvalue,BigDecimal pvalue,boolean contain){
