package com.zhikuntech.intellimonitor.windpowerforecast.domain.service.impl;

import com.zhikuntech.intellimonitor.windpowerforecast.domain.entity.WfAssessChange;
import com.zhikuntech.intellimonitor.windpowerforecast.domain.mapper.WfAssessChangeMapper;
import com.zhikuntech.intellimonitor.windpowerforecast.domain.service.IWfAssessChangeService;
import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import org.springframework.stereotype.Service;

/**
 * <p>
 *  服务实现类
 * </p>
 *
 * @author liukai
 * @since 2021-06-21
 */
@Service
public class WfAssessChangeServiceImpl extends ServiceImpl<WfAssessChangeMapper, WfAssessChange> implements IWfAssessChangeService {

}
