...
(
    ...
    logic_partition  varchar(256)  null comment '逻辑分区(汇总一个页面的所有的测点)'
)
    comment '测点';
...
(
    ...
    chain_info      varchar(64)    null comment '标识告警唯一链',
    ...
    alarm_describe  varchar(1024)  null comment '告警描述',
    alarm_level     varchar(64)    null comment '告警等级(严重/一般/告知)',
    constraint alarm_produce_info_idx_row_stamp
        unique (row_stamp)
)
    comment '告警信息';
...
create table mp_structure_to_golden_avg
(
    id         int auto_increment comment '主键id'
        primary key,
    golden_id  int null comment '庚顿id',
    backen_id  int null comment '本地id',
    fan_number int null comment '风机编号',
    data_type  int null comment '1:加速度.2:沉降'
);

create table mp_structure_to_golden_max
(
    id         int auto_increment comment '主键id'
        primary key,
    golden_id  int null comment '庚顿id',
...
create table mp_structure_to_golden_min
(
    id         int auto_increment comment '主键id'
        primary key,
    golden_id  int null comment '庚顿id',
...
create table sedimentation_data
(
    id         int auto_increment
        primary key,
    fan_number int           null comment '风机编号',
    value1     double(11, 4) null comment '沉降1',
    value2     double(11, 4) null comment '沉降2',
    value3     double(11, 4) null comment '沉降3',
    value4     double(11, 4) null comment '沉降4'
);
...
create index calc_date_newest_uindex
    on wf_analyse_cdq (calc_date, newest);
...
create index wf_assess_change_day_ref_id_newest_uindex
    on wf_assess_change (day_ref_id, newest);
...
create index calc_date_uindex
    on wf_assess_month (calc_date);
...
create index header_date_event_date_time_high_level_uindex
    on wf_data_cf (high_level);
